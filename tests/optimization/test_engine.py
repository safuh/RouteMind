...
from routemind.domain.models import Location, OptimizationPolicy, Package, Shipment, TransportCapacity, TransportLeg, TransportMode, TransportOption, TransportPrice, TransportSchedule
...
def shipment(id_: str, origin: Location, destination: Location, weight: float) -> Shipment:
    return Shipment(
        ...
        packages=[Package(id=f"P-{id_}", weight_kg=weight, length_m=.2, width_m=.2, height_m=.2)],
        ready_at=datetime(2026, 8, 27, 7, tzinfo=UTC), deadline=datetime(2026, 8, 27, 20, tzinfo=UTC),
    )


def option(id_: str, origin: Location, destination: Location, departure: datetime, cost: str, capacity: float) -> TransportOption:
...
def path(shipment_id: str, option_id: str, origin: Location, destination: Location, departure: datetime, weight: float, cost: str) -> CandidatePath:
    arrival = departure + timedelta(hours=3)
    leg = TransportLeg(option_id=option_id, origin=origin, destination=destination, departure_at=departure, arrival_at=arrival, allocated_weight_kg=weight, allocated_volume_m3=.008)
    return CandidatePath(
        shipment_id=shipment_id, legs=(leg,), total_cost=Decimal(cost), currency="KES", transit_seconds=10800,
        waiting_seconds=0, number_of_transfers=0, reliability=.95, modes=(TransportMode.BUS,), providers=("P",),
        capacity_utilization=weight / 100, deadline_feasible=True,
    )
...
def test_optimizer_assigns_one_path_per_shipment_and_minimizes_cost():
    ...
    paths = [path("A", "BUS-1", nbo, nku, t, 10, "100"), path("A", "BUS-2", nbo, nku, t, 10, "40"), path("B", "BUS-1", nbo, nku, t, 10, "100"), path("B", "BUS-2", nbo, nku, t, 10, "40")]
...
def test_optimizer_enforces_shared_scheduled_capacity():
    ...
    paths = [
        path("A", "BUS-1", nbo, nku, t, 10, "50"), path("A", "BUS-2", nbo, nku, t, 10, "100"),
        path("B", "BUS-1", nbo, nku, t, 10, "50"), path("B", "BUS-2", nbo, nku, t, 10, "100"),
    ]
