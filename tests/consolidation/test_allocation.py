...
from routemind.domain.models import (
    Location, Package, PricingModel, Shipment, TransportCapacity,
    TransportLeg, TransportMode, TransportOption, TransportPrice, TransportSchedule,
)
...
def _shipment(id_: str, destination: str, locations):
    return Shipment(
        id=id_, origin=locations["NBO"], destination=locations[destination],
        packages=[Package(id=f"p-{id_}", weight_kg=10, length_m=.1, width_m=.1, height_m=.1)],
...
def _option(id_: str, origin, destination, departure, arrival, price):
    return TransportOption(
        id=id_, provider_id="carrier", provider_name="Carrier", mode=TransportMode.TRUCK,
...
def _path(shipment_id, legs):
    return CandidatePath(
        shipment_id=shipment_id, legs=tuple(legs), total_cost=Decimal("100"), currency="KES",
        transit_seconds=sum((leg.arrival_at - leg.departure_at).total_seconds() for leg in legs),
        waiting_seconds=0, number_of_transfers=len(legs) - 1, reliability=.99,
        modes=tuple(TransportMode.TRUCK for _ in legs), providers=tuple("carrier" for _ in legs),
...
def test_shared_upstream_leg_and_private_downstream_legs_are_separated():
    ...
    legs_a = [TransportLeg(option_id=shared.id, origin=loc["NBO"], destination=loc["KLA"], departure_at=t0, arrival_at=t1),
              TransportLeg(option_id=a_down.id, origin=loc["KLA"], destination=loc["KGL"], departure_at=t2, arrival_at=t2 + timedelta(hours=2))]
    legs_b = [TransportLeg(option_id=shared.id, origin=loc["NBO"], destination=loc["KLA"], departure_at=t0, arrival_at=t1),
              TransportLeg(option_id=b_down.id, origin=loc["KLA"], destination=loc["GOM"], departure_at=t2, arrival_at=t2 + timedelta(hours=3))]
    shipments = {"A": _shipment("A", "KGL", loc), "B": _shipment("B", "GOM", loc)}
    allocation = allocate_consolidation(
        (_path("A", legs_a), _path("B", legs_b)), shipments,
...
def test_same_downstream_schedule_is_shared_too():
    ...
    def legs():
        return [
            TransportLeg(option_id=shared1.id, origin=loc["NBO"], destination=loc["KLA"], departure_at=t0, arrival_at=t1),
            TransportLeg(option_id=shared2.id, origin=loc["KLA"], destination=loc["KGL"], departure_at=t2, arrival_at=t2 + timedelta(hours=2)),
        ]
    shipments = {"A": _shipment("A", "KGL", loc), "B": _shipment("B", "KGL", loc)}
    allocation = allocate_consolidation(
        (_path("A", legs()), _path("B", legs())), shipments,
