...
class PathSearchEngine:
    ...
    def discover_paths(self, shipment: Shipment) -> list[CandidatePath]:
        ...
        if not shipment_timing_ok(shipment):
            self._record(
                rejected,
                PathRejection(
                    reason=RejectionReason.INVALID_SHIPMENT_TIMING,
                    message="Shipment deadline must be after readiness",
                ),
            )
            ...
            for option in self._by_origin.get(state.location_id, []):
                if option.id in used_option_ids:
                    self._record(
                        ...
                        PathRejection(
                            ...
                            message="Transport option already occurs in this path",
                        ),
                    )
                    continue
                ...
                if policy_reason:
                    self._record(
                        ...
                        PathRejection(
                            option_id=option.id,
                            reason=policy_reason[0],
                            message=policy_reason[1],
                        ),
                    )
                    continue
                if not option.available:
                    self._record(
                        ...
                        PathRejection(
                            ...
                            message="Transport option is unavailable",
                        ),
                    )
                    continue
                if not self._cargo_compatible(shipment, option):
                    self._record(
                        ...
                        PathRejection(
                            option_id=option.id,
                            reason=RejectionReason.CARGO_RESTRICTION,
                            message="Shipment is incompatible with transport restrictions",
                        ),
                    )
                    continue
                if self._config.max_transfers is not None and len(state.legs) - 1 >= self._config.max_transfers:
                    self._record(
                        ...
                        PathRejection(
                            option_id=option.id,
                            reason=RejectionReason.MAX_TRANSFERS,
                            message="Maximum transfer policy reached",
                        ),
                    )
                    continue
                if not self._price_supported(option):
                    self._record(
                        ...
                        PathRejection(
                            option_id=option.id,
                            reason=RejectionReason.UNSUPPORTED_PRICING,
                            message=f"Pricing model {option.price.model.value} requires distance_km",
                        ),
                    )
                    continue
                ...
                for schedule in option.schedules:
                    ...
                    if not transport_can_carry_shipment(
                        ...
                        reason = (
                            ...
                            else RejectionReason.VOLUME_CAPACITY
                        )
                        self._record(
                            rejected,
                            PathRejection(
                    ...
                    if reliability < self._config.min_reliability:
                        self._record(
                            ...
                            PathRejection(
                                option_id=option.id,
                                schedule_departure=schedule.departure_at.isoformat(),
                                reason=RejectionReason.RELIABILITY,
                                message="Compounded path reliability is below the configured threshold",
                            ),
                        )
                        continue
                    leg = TransportLeg(
                    ...
                    cost = state.cost + leg_cost + (
                        self._config.transfer_handling_cost if state.legs else Decimal("0")
                    )
                    ...
                    heappush(
                        queue,
                        (
                            cost + Decimal(str(schedule.transit_seconds / 3600 * 0.01)),
                            counter,
                            next_state,
                        ),
                    )
        ...
        if limit_reached:
            self._record(
                rejected,
                PathRejection(
                    reason=RejectionReason.SEARCH_LIMIT,
                    message="Maximum path-search expansion budget reached",
                ),
            )
    ...
    def _finish(
        ...
        self.last_diagnostics = PathSearchDiagnostics(
            ...
            expansion_limit_reached=limit_reached,
        )
    ...
    def _build_candidate(self, shipment: Shipment, state: _State) -> CandidatePath:
        return CandidatePath(
            ...
            emissions_kg_co2e=state.emissions_kg_co2e if state.emissions_kg_co2e > 0 else None,
            metadata={"search": "time_dependent", "pricing": "distance_aware"},
        )
    ...
    def _policy_rejection(self, option: TransportOption) -> tuple[RejectionReason, str] | None:
        if option.mode in self._config.excluded_modes or (
            self._config.allowed_modes is not None and option.mode not in self._config.allowed_modes
        ):
            return RejectionReason.POLICY_MODE, "Transport mode is excluded by search policy"
        if option.provider_id in self._config.excluded_provider_ids or (
            self._config.allowed_provider_ids is not None
            and option.provider_id not in self._config.allowed_provider_ids
        ):
    ...
    def _cargo_compatible(shipment: Shipment, option: TransportOption) -> bool:
        ...
        if "no_temperature_controlled" in restrictions and any(
            ...
            return False
        if "fragile_only" in restrictions and not all(
            package.fragile for package in shipment.packages
        ):
            return False
    ...
    def _schedule_rejection(
        ...
        if state.legs and schedule.departure_at <= state.legs[-1].arrival_at + timedelta(
    ...
    def _price_supported(option: TransportOption) -> bool:
        ...
        return model in {"fixed", "per_kg", "per_volume", "per_km", "per_kg_km", "quoted"} and (
            model not in {"per_km", "per_kg_km"} or option.distance_km is not None
        )
