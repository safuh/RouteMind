...
class PathSearchEngine:
    ...
    def discover_paths(self, shipment: Shipment) -> list[CandidatePath]:
        ...
        counter = 0
        heappush(queue, (Decimal("0"), counter, _State(shipment.origin.id, shipment.ready_at, (), Decimal("0"), 1.0, 0.0, 0.0, 0.0)))
        ...
        while queue and expansions < self._config.max_expansions:
            ...
            for option in self._by_origin.get(state.location_id, []):
                ...
                if self._config.max_transfers is not None and len(state.legs) >= self._config.max_transfers + 1:
                    self._record(rejected, PathRejection(option_id=option.id, reason=RejectionReason.TRANSFER_TIME, message="Maximum transfer policy reached"))
                    continue
                if not self._price_supported(option):
                    self._record(rejected, PathRejection(option_id=option.id, reason=RejectionReason.UNSUPPORTED_PRICING, message=f"Pricing model {option.price.model.value} requires distance-aware economics"))
                    continue
                ...
                for schedule in option.schedules:
                    ...
                    if not transport_can_carry_shipment(shipment, option, available_weight_kg=available_weight, available_volume_m3=available_volume):
                        reason = RejectionReason.WEIGHT_CAPACITY if available_weight is not None and shipment.weight_kg > available_weight else RejectionReason.VOLUME_CAPACITY
                    ...
                    weight_capacity = min(option.capacity.max_weight_kg, available_weight if available_weight is not None else option.capacity.max_weight_kg)
                    ...
                    if available_volume is not None:
                        volume_capacity = available_volume if volume_capacity is None else min(volume_capacity, available_volume)
                    ...
                    cost = state.cost + self._price_for_shipment(option, shipment) + (self._config.transfer_handling_cost if state.legs else Decimal("0"))
                    next_state = _State(option.destination.id, schedule.arrival_at, state.legs + (leg,), cost, reliability, state.transit_seconds + schedule.transit_seconds, state.waiting_seconds + waiting, max(state.capacity_utilization, utilization))
                    counter += 1
