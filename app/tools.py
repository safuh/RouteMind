...
from routemind.domain.models import OptimizationPolicy, OptimizationResult, TransportOption, TransportPlan
...
def extract_optimization_policy(request: str) -> dict[str, Any]:
    """Normalize a business objective request into a deterministic policy.

    This helper is intentionally conservative: the ADK agent can explain the
    interpretation, but this tool only maps explicit business keywords to
...
def optimize_portfolio_json(
    ...
    # ConsolidationOpportunity is deliberately imported lazily to keep this tool's
    # surface small while preserving the same deterministic model boundary.
...
def validate_optimization_result(candidate_paths_json: str, result_json: str) -> str:
    ...
    known = {
        (path.shipment_id, path.option_ids, path.currency)
        for path in paths
        if path.deadline_feasible
    }
    seen: set[str] = set()
    errors: list[str] = []
